function hesapla() {
  ...
  if (gelen_fiyat == "" && indirim == "") {
    ...
    alert("Ürün indirimini doldurun");
    return;
  }
  ...
  if (indirimli_fiyat > indirim) {
    ...
    if (indirimli_fiyat > indirim) {
      ...
      let son_sonuc = document.createTextNode(
        "Ve indirimli hali: " + indirimli_fiyat.toFixed(2) + "\n",
      );
      kart.appendChild(son_sonuc);
      let indirim_hali = document.createTextNode(
        "Yapılan indirim: " + indirim.toFixed(2) + "\n",
      );
      kart.appendChild(indirim_hali);
      let gelen_hali = document.createTextNode(
        "İndirimsiz hali: " + gelen_fiyat.toFixed(2) + "\n",
      );
